...
namespace WallFinishByCategory_v1.View
{
    ...
    public partial class UserWindow : Window
    {
        ...
        //************************************************************************************
        public UserWindow(ExternalCommandData commandData, Document doc, Document docNow, bool worksharedStatus, ICollection<Element> allRoomsCollection, List<WallType> allWallType)
        {
            InitializeComponent();
        ...
        private void CategoryBtn_Click(object sender, EventArgs e)
        {
            ...
            foreach (string category in allWallCategories)
            {
                ...
                //create combobox
                System.Windows.Controls.ComboBox wallTypeSelecter = new System.Windows.Controls.ComboBox();
        ...
        private void StartCreateNewWall(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                foreach (Room room in roomBySelectedCategoryLevelAndItems)
                {
                    ...
                    using (Transaction t = new Transaction(_docNow))
                    {
                        t.Start("NewWallFinish");
                        allWallNewAxis = RoomWorker.GetNewWallAxisAll(_docNow, room, roomCurves, defaultWallType);
                        ...
                        foreach (Curve axis in allWallNewAxis)
                        {
                            ...
                            Wall newWall = null;
                            newWall = RoomWorker.CreateNewWallByCurve(_docNow, room, axis, defaultWallType,
                                ROOM_NUMBER_PARAM, ROOM_NAME_PARAM, selectedWorksetElementId);
                            newWalls.Add(newWall);
                            //join with closed room wall

                            Wall roomWallToJoin = RoomWorker.FindClosedRoomWall(_docNow, newWall, roomWalls);
                            ...
                            var filterDoors = new ElementCategoryFilter(BuiltInCategory.OST_Doors);
                            var filterWindows = new ElementCategoryFilter(BuiltInCategory.OST_Windows);

                            List<ElementId> doorsId = roomWallToJoin.GetDependentElements(filterDoors).ToList();
                            List<ElementId> windowsId = roomWallToJoin.GetDependentElements(filterWindows).ToList();

                            List<ElementId> lst = new List<ElementId>();
                            lst.AddRange(doorsId);
                            lst.AddRange(windowsId);

                            if (lst.Count > 0)
                            {
                                foreach (var elId in lst)
                                {
        ...
        private WallType GetNeedWallTypeFromSelected(string categoryName)
        {
            ...
            foreach (System.Windows.Controls.Panel p in allPanels)
            {
                ...
                {
                    resultType = (p.Children[1] as System.Windows.Controls.ComboBox).SelectedItem as WallType;
                    break;
                }
            }
            return resultType;
        ...
        static List<XYZ> GetTwoPointsWithMaxDistance(List<XYZ> xYZs,Document document)
        {
            ...
            List<XYZ> list = new List<XYZ> { one, two };
            return list;
        }
    }
    ...
    public class RoomWarningSwallower : IFailuresPreprocessor
    {
        public FailureProcessingResult PreprocessFailures(FailuresAccessor failuresAccessor)
        {
            ...
            /// Прямоугольный проем не образует выреза в основе
            if (failList.Where(x => x.GetFailureDefinitionId() == BuiltInFailures.OpeningFailures.OpeningRectDoesnotCutHost).Count() > 0)
            {
                failuresAccessor.ResolveFailures(failList);
